			...
			if xTarget then
				if deadPlayers[playerId] then
					if Config.ReviveReward > 0 then
						...
						TriggerEvent('7DiscordLog:AmbulanceLog', xPlayer, 'Revive', xTarget, Config.ReviveReward)
					else
						xPlayer.showNotification(_U('revive_complete', xTarget.name))
						xTarget.triggerEvent('esx_ambulancejob:revive')
						TriggerEvent('7DiscordLog:AmbulanceLog', xPlayer, 'Revive', xTarget)
					end
				else
...
AddEventHandler('esx_ambulancejob:heal', function(target, type)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		TriggerEvent('7DiscordLog:AmbulanceLog', xPlayer, 'Heal', xTarget)
	end
end)
