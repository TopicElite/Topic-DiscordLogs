	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
				...
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Remove', societyName, amount)
			else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
				...
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Add', societyName, amount)
			end)
		else
			xPlayer.showNotification(_U('invalid_amount'))
		end
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			function(rowsChanged)
				...
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Wash', societyName, amount)
			end)
		else
			xPlayer.showNotification(_U('invalid_amount'))
		end
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
				...
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Hire', societyName, xTarget)
			elseif type == 'promote' then
				xTarget.showNotification(_U('you_have_been_promoted'))
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Promote', societyName, xTarget)
			elseif type == 'fire' then
				xTarget.showNotification(_U('you_have_been_fired', xTarget.getJob().label))
				TriggerEvent('7DiscordLog:SocietyLog', xPlayer, 'Fire', societyName, xTarget)
			end
